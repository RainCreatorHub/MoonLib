...
-- Função CreateFolder modificada para criar Settings.json
function RainLib:CreateFolder(folderName)
    ...
    if makefolder and writefile then
        if not isfolder(folderName) then
            ...
            -- Criar o arquivo Settings.json dentro da pasta
            ...
            local jsonSettings = game:GetService("HttpService"):JSONEncode(defaultSettings)
            writefile(settingsPath, jsonSettings)
            print("[RainLib] Arquivo 'Settings.json' criado em: " .. settingsPath)
            self:Notify({
                Title = "Sucesso",
                Content = "Arquivo 'Settings.json' criado em '" .. folderName .. "'!",
                Duration = 3
            })
            return true
        else
            ...
            -- Verifica se Settings.json já existe, caso contrário cria
            local settingsPath = folderName .. "/Settings.json"
        ...
        warn("[RainLib] Este executor não suporta as funções makefolder ou writefile!")
        self:Notify({
            Title = "Erro",
            Content = "Executor não suporta criação de pastas ou arquivos!",
            Duration = 3
        })
        return false
    end
